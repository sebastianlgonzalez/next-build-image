...
function generateLoader(
  ...
  return [
    ...
    `  const normalizedSrc = src.slice(0, src.lastIndexOf(".")).toLowerCase() + "." + ext;`,
    `  const withoutExt = normalizedSrc.slice(0, normalizedSrc.lastIndexOf("."));`,
    `  const available = optimizedWidths[normalizedSrc];`,
    `  const isUnresized = unresized.includes(normalizedSrc);`,
